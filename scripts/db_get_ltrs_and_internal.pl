...
print OUTFILE "feature_id\tltr_1\tltr_2\tinternal_seq\n";
...
for my $id (@ids) {
	...
	for my $child (@children) {
		...
		if ($ltr_count == 1) {
		    $ltr_1_start = $child->start;
		    $ltr_1_end = $child->end;
		    $line_out = $line_out."\t".$child_seq;
		    
		} 
		elsif ($ltr_count == 2) {
		    $ltr_2_start = $child->start;
		    $ltr_2_end = $child->end;
		    $line_out = $line_out."\t".$child_seq;
		    ...
		    print STDERR "WARNING: More LTRs than expected\n";
		}
	...
	#print 
	if ( $ltr_count < 2 ) {
	    print STDERR "WARNING: FEWER LTRs than expected\t";
