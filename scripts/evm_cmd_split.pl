...
while (<INFILE>) {
    ...
    print STDERR "Pocessing line $line_num to file $i \n";
    
    # Set the path for the outfile name
    my $out_path = $OutputDir.$outfile.$i.".list";
    
    open (OUTFILE, ">>$out_path") ||
	die "Can not open outfile $out_path";

    print OUTFILE $_."\n";

    # The loop to write the outfiles
    $i++;
    $line_num++;
