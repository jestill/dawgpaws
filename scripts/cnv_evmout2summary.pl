...
#-----------------------------------------------------------+
#                                                           |
# cnv_evmout2summary.pl - Summarize EVM evidences used      |
#                                                           |
#-----------------------------------------------------------+
#                                                           |
...
#                                                           |
# USAGE:                                                    |
#  cnv_evmout2summary.pl -i evmFile.evm.out -o summ.txt     |
#                                                           |
# VERSION: $Rev$                                            |
#                                                           |
#                                                           |
...
# ITERATE ACROSS THE INFILE
my $evm_model_count = 0;
while (<INFILE>) {
    chomp;
    
    if ( m /^\#/ ) {
	$evm_model_count++;
#	print STDERR "NEW MODEL $evm_model_count\n";

	# Can parse model information here before moving on
	next;
    }
	...
	foreach my $evidence ( @evidences ) {
	    ...
	    $evidence =~ s/\{//;
	    $evidence =~ s/\}//;
	    my ($model, $program) = split (/\;/, $evidence);

	    print OUTFILE "evm".$evm_model_count."\t".
		$program."\n";
	}
...
print STDERR $evm_model_count." models processed\n";
