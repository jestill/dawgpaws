...
#  Allows the passing of passwords with Expect.             | 
#                                                           |
# DEPENDENCIES:                                             |
...
#
# DOES NOT WORK
# GET A LOST CONNECTION ERROR : Name or service not known
#
...
my $UserName = $Options{u} ||
    die "You must provide a user name for the SCP connection.\n$Usage\n";
my $Quiet = $Options{q};
...
my $LocFile = "/home/jestill/test.txt";
my $RemFile = "/Users/ateam/.apollo/wheat.tiers";
my $RemDir = "/Users/ateam/.apollo";
...
#    $scpe->login( $UserName, $UserPassword );
    
    my $scpe = Net::SCP::Expect -> new ( host=> '$IndTarget',
					 user=> '$UserName',
					 password=> '$UserPassword')
	|| die "Count not make connection to: $IndTarget\n";
    
    # Copy file to remote directory 
#    $scpe->scp( '$LocFile', '$RemDir' ) ||
#	die "Cound not copy from:\n\t$LocFile\nTO:\n\t$RemDir\n" ;

    $scpe->scp('/home/jestill/test.txt', '/Users/ateam/');
