...
my $BaseDir = "/home/jestill/projects/wheat_annotation/VennGeneRep/";
...
foreach $FileRoot (@Files2Proc)
{
    ...
    my $SvgOut = $BaseDir.$FileRoot."svg";
...
#    my $SysCmd = "$prog -i $InFile -o $OutFile -d $DatDir -q -f tab -s $SvgOut > $ProgOut";
    my $SysCmd = "$prog -i $InFile -o $OutFile -d $DatDir -q -f tab -s $SvgOut;
#my $SysCmd = "$prog";
