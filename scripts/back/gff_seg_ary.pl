...
while (<GFFIN>) {
    ...
    if ($num_cols == 9) {
	$cur_bin_val = &get_bin_val($gff_data[5], @thresh_vals);
	print STDERR $cur_bin_val."\n" if $verbose;
    }

    # Err msg while testing
    print STDERR $line_count.":".$gff_data[5].
	"-".$cur_bin_val."\n" if $verbose;
	...
	# For man applications NULL may be the most common option
	# so putting this first
	if ($in_seg == 1) {
	    &print_seg ($gff_data[0],
			$gff_data[1],
			$gff_data[2],
			$seg_start,
			$seg_end,
	...
	if ($cur_bin_val =~ $prev_bin_val) {
		...
		# Print old values and reset coords
		&print_seg ($gff_data[0],
			    $gff_data[1],
			    $gff_data[2],
			    $seg_start,
			    $seg_end,
			    $prev_bin_val,
			    "test segment from ne",);

	    }

#	    if ($prev_bin_val =~ "NULL") {
#		
#	    }
#	    else {
#		if ($cur_bin_val > $prev_bin_val) {
#		    # remove overlaps
#		    # set end of prev tile to 
#		}
#
#		
#	    }

	    # set new seg_start and end value
	    $seg_start = $gff_data[3];
	    $seg_end = $gff_data[4];
	    $seg_count++;
	    $in_seg = 1;
...
#
# OLD SINGLE VALUE OR BIN THRESHOLD BELOW
#-----------------------------+
# PROCESS GFF FILE            |
#-----------------------------+
my $seg_count = 0;
while (<GFFIN>) {
	...
	if ( $in_score >= $thresh ) {
		...
		# Append seg count
		if ($param) {
		    $attribute = $attribute."_".$param;
		}
		$attribute = $attribute."_".$seg_count;
