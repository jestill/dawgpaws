...
if ($Format =~ "tab")
{
    ...
    {
	$FileTestNum++;
	$FileSize = -s $FeatDir.$FileTest || 0;
	print $FileTest." ".$FileSize."\n";
...
#	if ($FileSize == 0)
	...
	for (my $i++; $i<=$EmptyFileNum;$i++)
	{
	    print $EmptyFiles[$i][1]."\t";
	    print $EmptyFiles[$i][2]."\t";
	    print $EmptyFiles[$i][3]."\n";
	}
    }else{
	# I can get rid of this else when things are working
	print "Empty File num is: $EmptyFileNum\n";
    }
