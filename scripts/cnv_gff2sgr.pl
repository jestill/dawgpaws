...
cnv_gff2sgr.pl - Convert GFF file to SGR format
...
=head1 SYNOPSIS
  ...
  USAGE:
    cnv_gff2sgr.pl -i infile.gff3 -o outfile.sgr

    --infile        # Path to the input file in GFF format
    --outfie        # Path to the output file in SGR foramt
...
SGR is a Tab-delimited graph format. Sequence graph files that show 
base coordinate scores. These files are generated by CNAT (the Affymetrix 
Chromosome Copy Number Analysis Tool software). The format of .sgr text 
files is: chromosome identification, then two columns of numbers 
separated by a single space or a tab. The first number is the 
base position; the second number is the score
...
Path of the input file in GFF format.

=item -o,--outfile

Path of the output file in SGR format. This format is supported by the
Apollo genome annotation curation program.
...
cnv_gff2sgr.pl -i infile.gff3 -o outfile.gff3
...
This program does not make use of a configuration file or any variables
