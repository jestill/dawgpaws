...
#-----------------------------------------------------------+
#                                                           |
# cnv_otho2tab.pl - Convert ortho groups to table           |
#                                                           |
#-----------------------------------------------------------+
#                                                           |
...
while (<INFILE>) {
    chomp;
    my @ortho_parts = split;
    
    my $i = -1;
    my $ortho_group;
    foreach my $gene (@ortho_parts) {
	my $species;
	$i++;
	if ($i == 0) {
	    if ( $ortho_parts[0] =~ m/(.*)\:/ ) {
		...
		$ortho_group = $ortho_parts[0];
	    }
	    next;
	}
	else {
	    my @gene_parts = split ( /\|/, $ortho_parts[$i]);
	    my $genome = substr ($gene_parts[1], 0, 5);
	    print OUTFILE $ortho_group."\t".
		$gene_parts[1]."\t".
		$gene_parts[2]."\t".
		$ortho_parts[$i]."\n";
