...
# Initially will just support a single feature at a time
# but keep this if I want to build on it later
# the list of features to search for in each bin
my $search_feature = "gap";    # By default will evaluate gaps
my $report_value = "count";    # The value to report in output
my $max_chrom;                 # Only report up to max_chrom scaffolds
# Open a separate output file for each feature value combination
...
#		    "summary=s"    => \$summary_outfile,
		    "f|feature=s"  => \$search_feature,
		    "value=s"      => \$report_value,
...
# Try to open a filehandle for each feature
# in the array of features to process.
...
#    my $scaffold_length = $id->length;
	...
	# Dropped array refernce here 
	# just doing a single feature type
	...
	# If we want to use color, we will need to load the set of
	# values to an array, and then pass to the R function
	# for assigning a color
	...
	# Need to modify the type of value that is reported
	# based on input from the command line
	# possible options will be
	#   - count
	#   - composition (length in bp)
	#   - ratio (length as a ratio)

	my $out_value;
	if ($report_value =~ "count") {
	    $out_value = $num_features;
	}
	elsif ($report_value =~ "composition") {
	    $out_value =$feature_composition;
	}
	elsif ($report_value =~ "ratio" ) {
	    $out_value = $feature_ratio;
	}
