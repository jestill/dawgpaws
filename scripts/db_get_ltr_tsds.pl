...
print OUTFILE "feature_id\ttsd_1\ttsd_2\ttsd_match\n";
...
for my $id (@ids) {
	...
	my $tsd_ltr_id = $feature->primary_id;
	
	for my $child (@children) {
#	    print STDOUT "$child\n";
	    if ($child =~ "target_site_duplication" ) {
		
		$tsd_count++;
		my $child_seq = $db->fetch_sequence($id,
						    $child->start, 
						    $child->end);	    

		if ($tsd_count == 1) {
		    $tsd_1_start = $child->start;
		    $tsd_1_end = $child->end;
		    $tsd_1_seq = $child_seq;
		    $line_out = $line_out."\t".$child_seq;
		    $tsd_1_length = length($tsd_1_seq);
		    
		} 
		elsif ($tsd_count == 2) {
		    $tsd_2_start = $child->start;
 		    $tsd_2_end = $child->end;		    
		    $tsd_2_seq = $child_seq;
		    $tsd_2_length = length($tsd_2_seq);
		    $line_out = $line_out."\t".$child_seq;
		    
		    if ($tsd_1_seq =~ $tsd_2_seq) {
			$tsd_match = "Y";
			$line_out = $line_out."\tY";
		    }
		    else {
...
#		    # Now get internal domain sequence here
#		    my $lrp_id_seq =  $db->fetch_sequence($id,
#							  ($tsd_1_end + 1), 
#							  ($tsd_2_start - 1));
#		    $line_out = $line_out."\t".$lrp_id_seq;
		    

		}
		else {
		    print STDERR "WARNING: More TSDs than expected\n";
		}
		
		
	    }
	    # Some programs set TSD as part of parent feature htat
	    # is different than LTR
	    elsif ($child =~ "LTR_retrotransposon") {
		# NEED TO GET THE CHILD ID HERE???
		$tsd_ltr_id = $child->primary_id;
		print STDERR "Found LTR Primary ID is ".
		    $tsd_ltr_id."\n" if $verbose;
	    }# End of if child equal TSD
	...
	#print 
	if ( $tsd_count == 0 ) {
	    print STDERR "No TSDS annotated\n"
		if $verbose;
	}
	elsif ( $tsd_count < 2 ) {
	    print STDERR "WARNING: FEWER TSDs than expected\t";
	    ...
	    print STDERR "TSDs okay\t"
		if $verbose;
	    print STDERR $feature."\n"
		if $verbose;
	    print OUTFILE $tsd_ltr_id."\t".$tsd_1_seq."\t".
		$tsd_2_seq."\t".$tsd_match."\t".
		$tsd_1_length."\t".$tsd_2_length."\n";

# 	    print OUTFILE $tsd_match

#
#	    print OUTFILE $line_out."\n";
	}
