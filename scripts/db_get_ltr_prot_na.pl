...
for my $id (@ids) {
	...
	# Get all the subfeatures of the ltr_retrotransposon
	# The protein matches are for the ltr_harvest reslts
	# we will want to fetch the name as name=tag
	my @subfeatures = $feature->get_SeqFeatures(-type=> 'protein_match');

	for my $subfeature (@subfeatures) {
	    print STDERR "\t\t\t\t".$subfeature."\n"
		if $verbose;

	    # We want to get the name attribute
	    if (my $name = ($subfeature->attributes('name'))[0]) {
		#print STDERR "Has existing spatial cluster. Will not update\n";
		print STDERR "\t\t\t\tNAME:".$name."\n"
		    if $verbose;


		print OUTFILE $feature->primary_id."=".$name."\n";
		# Then from here we want to print to output file
		# either as list or as boolean for node attributes in cytoscape
	    }


	}

	# I used the transposable_element_gene 
	# tag in dawgpaws for other programs
	# I also used cap N for Name tag
	my @dp_subfeatures  = $feature->get_SeqFeatures(-type=>'transposable_element_gene');
	for my $dp_subfeature (@dp_subfeatures) {
	    if (my $name = ($dp_subfeature->attributes('Name'))[0]) {
		print OUTFILE $feature->primary_id."=".$name."\n";
	    }
	}
	
    } # End of for each subfeature
