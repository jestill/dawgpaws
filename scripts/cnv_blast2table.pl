...
#-----------------------------------------------------------+
#                                                           |
# cnv_blast2table.pl - Convert tiled BLAST to summary files |
#                                                           |
#-----------------------------------------------------------+
#                                                           |
...
#-----------------------------+
#blastxml
#my $blio = Bio::SearchIO->new( -format => 'blasttable',

# print header

if ($outfile) {
    open (TABOUT, ">$outfile") ||
	die "Can not open outfile file\n";
}
else {
    open (TABOUT, ">&STDOUT") ||
...
my $blio = Bio::SearchIO->new( -format => 'blastxml',
                               -file   => $infile) ||
    die "Coult not open".$infile;

my $count = 0;
while (my $result = $blio->next_result) {
    $count++;
    print STDERR "Processing result: ".$count."\n" 
	if $verbose;
    
    my $hit_counter = 0;
    while (my $hit = $result->next_hit) {
	$hit_counter++;
	print STDERR "\thit:".$hit_counter."\n"
	    if $verbose;

	my $tiling =
	    Bio::Search::Tiling::MapTiling->new($hit);

	my $align_percent = $tiling->frac_aligned(-type=> 'query') * 100;
	
	print TABOUT $result->query_name."\t".
	    $result->query_length."\t".
	    $hit->name."\t".
	    $hit->length."\t".
	    $tiling->range()."\t".
