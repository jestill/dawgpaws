...
#  intput directory using the EMBOSS dottup program. This   |
...
batch_dottup.pl - Run the EMBOSS dottup program in batch mode.
...
=head1 SYNOPSIS
  ...
  USAGE:
    batch_dottup.pl -i input_dir -o out_dir

    -i,--indir         # Path to the input file
    -o,--outdir        # Path to the output file
    -w, wordsize       # Wordsize to build dotplot from
    -g, graph          # Type of graph

=head1 DESCRIPTION

Run the EMBOSS DOTTUP program in batch mode for a directory of FASTA
format files.
...
indicating the required information for different runs of the
batch dottup program.
...
This program does not currently make use of a separate configuration file.
...
This program requires the DOTTUP program
( L<http://emboss.open-bio.org/wiki/Appdoc:Dottup> (
that is part of the EMBOSS suite of programs
( L<http://emboss.sourceforge.net/> ).
