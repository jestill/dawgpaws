...
#-----------------------------------------------------------+
#                                                           |
# batch_find_rp3.pl - Find Matched to RNA Pol III           |
#                                                           |
#-----------------------------------------------------------+
#                                                           |
...
#                                                           |
# DESCRIPTION:                                              |
#  Given a directory of fasta files, this will find matches |
#  to RNA Pol III. Currently this will just search for      |
#  RRYNNRRYGG which is the 'A' Box of SINES.                |
#                                                           |
# LICENSE:                                                  |
...
for my $ind_file (@fasta_files) {
    ...
    my $gff_out = $dir_gff_out.$name_root."_rp3_a.gff";
    my $game_out = $dir_game_out.$name_root."_rp3_a.game.xml";
    ...
    #--------------------------------+
    # CHECK FOR RP3 B Box IN THE OBJ |
    #--------------------------------+
...
#	while ($seq_string =~ m/(($gap_char){$min_gap_len,})/g ) {
	# Goal is to find GGTTCGAnnCC

	#////////////////////////////////////////////
	# See if I can search for A Box here
	# A box search is RRYNNRRYGG
	while ($seq_string =~ m/([AG][AG][CT][AGCT][AGCT][AG][AG][AGCT]GG)/g ) {
	    ...
	    if ($gff_ver =~ "GFF3" ) {
		$attribute =  "ID=".$seq->primary_id."_SINE-A_".
		    $gap_len."-".
		    $gap_count;
	    }
	    else {
		$attribute =  $seq->primary_id."_SINE-A_".
		    $gap_len."-".
		    $gap_count;
	    }
	...
	while ($seq_string =~ m/(GGTTCGA[AGCT][AGCT]CC)/g ) {
	    ...
	    if ($gff_ver =~ "GFF3" ) {
		$attribute =  "ID=".$seq->primary_id."_SINE-B_".
		    $gap_len."-".
		    $gap_count;
	    }
	    else {
		$attribute =  $seq->primary_id."_SINE-B_".
		    $gap_len."-".
		    $gap_count;
	    }
...
This documentation refers to batch_findgaps version $Rev: 943 $
